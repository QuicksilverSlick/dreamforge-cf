 ...
 * Uses Gemini 3 Pro to analyze imported codebases and generate completion blueprints
 */
...
/**
 * Blueprint Generation Service using Gemini 3 Pro via Cloudflare AI Gateway
 */
export class BlueprintGenerationService {
    /**
     * Generate a completion blueprint using Gemini 3 Pro via AI Gateway
     */
    static async generateBlueprint(
        ...
        migrationAnalysis?: import('../../services/migration/cloudflare-compatibility-checker').MigrationAnalysisResult
    ): Promise<GeneratedBlueprint> {
        try {
            logger.info('Generating blueprint with Gemini 3 Pro via AI Gateway', {
            ...
            if (env.DEV_MODE === 'true') {
                ...
                return this.createFallbackBlueprint(context, migrationAnalysis);
            }
            ...
            const prompt = this.buildAnalysisPrompt(context, migrationAnalysis);

            // Call Gemini 3 Pro via Cloudflare AI Gateway
            const gatewayUrl = `https://gateway.ai.cloudflare.com/v1/${env.CF_ACCOUNT_ID}/${env.CF_AI_GATEWAY_ID}/google-ai-studio/v1/models/gemini-3-pro-preview:generateContent`;
            ...
            const blueprint = this.parseAIResponse(generatedText, context, migrationAnalysis);
...
${migrationAnalysis ? `# Cloudflare Workers Migration Analysis
**Platform Detected**: ${migrationAnalysis.platform}
**Migration Complexity**: ${migrationAnalysis.migrationComplexity}/100 (${migrationAnalysis.estimatedEffort} effort)
**Ready for Migration**: ${migrationAnalysis.readyForMigration ? 'Yes ✅' : 'No ❌'}

## Compatibility Issues Summary
- **Total Issues**: ${migrationAnalysis.issues.length}
- **Blockers**: ${migrationAnalysis.blockers.length}
- **High Priority**: ${migrationAnalysis.issues.filter(i => i.severity === 'high').length}
- **Medium Priority**: ${migrationAnalysis.issues.filter(i => i.severity === 'medium').length}
- **Low Priority**: ${migrationAnalysis.issues.filter(i => i.severity === 'low').length}
- **Auto-fixable**: ${migrationAnalysis.issues.filter(i => i.autoFixable).length}

## Required Cloudflare Configuration
${migrationAnalysis.requiredConfig.needsNodejsCompat ? '- ✅ nodejs_compat flag required' : ''}
${migrationAnalysis.requiredConfig.needsHyperdrive ? '- ✅ Hyperdrive (database proxy) required' : ''}
${migrationAnalysis.requiredConfig.needsR2 ? '- ✅ R2 object storage required' : ''}
${migrationAnalysis.requiredConfig.needsKV ? '- ✅ KV namespace required' : ''}

## Top Blockers
${migrationAnalysis.blockers.slice(0, 5).map(b => `- **${b.type}** in ${b.file}: ${b.message}`).join('\n')}

## Dependencies Analysis
${migrationAnalysis.dependencies.incompatible.length > 0 ? `
### Incompatible Packages (Must Replace)
${migrationAnalysis.dependencies.incompatible.slice(0, 10).map((p: { name: string; reason: string; alternative: string }) => `- ${p.name} - ${p.reason} (Alternative: ${p.alternative})`).join('\n')}
` : ''}
${migrationAnalysis.dependencies.requiresNodejsCompat.length > 0 ? `
### Packages Requiring nodejs_compat Flag
${migrationAnalysis.dependencies.requiresNodejsCompat.slice(0, 10).map((pkg: string) => `- ${pkg}`).join('\n')}
` : ''}
${migrationAnalysis.dependencies.recommended.length > 0 ? `
### Recommended Upgrades
${migrationAnalysis.dependencies.recommended.slice(0, 10).map((r: { current: string; recommended: string; reason: string }) => `- ${r.current} → ${r.recommended}: ${r.reason}`).join('\n')}
` : ''}
...
3. **Prioritized Recommendations**: Provide specific, actionable recommendations categorized by:
   ...
   ${migrationAnalysis ? '\n   **MIGRATION FOCUS**: Prioritize recommendations that address Cloudflare Workers blockers and compatibility issues identified above.' : ''}
...
Be specific, actionable, and optimistic about AI capabilities.`;
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                return {
                    ...
                    cloudflareMigration: migrationAnalysis ? {
                        platform: migrationAnalysis.platform,
                        complexity: migrationAnalysis.migrationComplexity,
                        effort: migrationAnalysis.estimatedEffort,
                        totalIssues: migrationAnalysis.issues.length,
                        blockers: migrationAnalysis.blockers.length,
                        highPriority: migrationAnalysis.issues.filter(i => i.severity === 'high').length,
                        mediumPriority: migrationAnalysis.issues.filter(i => i.severity === 'medium').length,
                        lowPriority: migrationAnalysis.issues.filter(i => i.severity === 'low').length,
                        autoFixable: migrationAnalysis.issues.filter(i => i.autoFixable).length,
                        readyForMigration: migrationAnalysis.readyForMigration,
                        topBlockers: migrationAnalysis.blockers.slice(0, 5).map(b => ({
                            type: b.type,
                            file: b.file,
                            message: b.message
                        })),
                        requiredConfig: {
                            needsNodejsCompat: migrationAnalysis.requiredConfig.needsNodejsCompat,
                            needsHyperdrive: migrationAnalysis.requiredConfig.needsHyperdrive,
                            needsR2: migrationAnalysis.requiredConfig.needsR2,
                            needsKV: migrationAnalysis.requiredConfig.needsKV
                        }
                    } : undefined
                };
            }
     ...
     */
    private static createFallbackBlueprint(
        context: CodebaseContext,
        migrationAnalysis?: import('../../services/migration/cloudflare-compatibility-checker').MigrationAnalysisResult
    ): GeneratedBlueprint {
        const todoCount = context.sourceFiles.reduce((acc, file) => acc + file.todos.length, 0);

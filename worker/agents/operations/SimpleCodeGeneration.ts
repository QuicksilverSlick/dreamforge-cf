...
// `GenerationContext` was a generic parameter on upstream's `AgentOperation`
// signature; the fork binds context to `GenerationContext` inside
// `OperationOptions` directly (see common.ts), so this import is unused
// after the signature adaptation above. Reintroduced when later M3 slices
// widen `common.ts` to the upstream generic shape.
...
// Upstream's `AgentOperation` is `<TContext, InputType, OutputType>` and
// `OperationOptions<TContext>` is generic. The fork's local versions
// (`worker/agents/operations/common.ts`) bind `TContext` to
// `GenerationContext` directly — they predate the generic refactor. Until
// later M3 slices widen `common.ts` to the upstream shape, this operation
// uses the fork's non-generic forms; the runtime semantics are identical
// because `OperationOptions.context: GenerationContext` is already what
// this code reads.
                ...
                onChunk: (chunk: string) => {
                    codeGenerationFormat.parseStreamingChunks(
                        ...
                        (filePath: string) => {
                            ...
                            const originalContents = context.allFiles.find((f: { filePath: string; fileContents: string }) => f.filePath === filePath)?.fileContents || '';

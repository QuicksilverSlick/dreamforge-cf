...
<TASK>
    ...
    **Visual Assets - Use These Approaches:**
    ✅ Generated images: Real brand/photo/illustration assets are generated by dedicated image models. Reference them by the \`url\` populated on each \`imageAssets\` entry in the blueprint (e.g. \`<img src="{url}">\`). To add or replace an image mid-build, call the \`generate_image\` tool — it generates and hosts the asset and wires it in on the next phase. Never author or reference local binary image files (.png/.jpg/.gif) yourself; they cannot be written from generated code.
    ✅ Canvas drawing: \`<canvas>\` element for shapes and patterns
    ✅ Icon libraries: lucide-react, heroicons (from dependencies)
    ✅ Inline SVG for simple vector shapes/decorations
    ✅ Audio: synthesize sound with the **Web Audio API** (OscillatorNode/AudioBuffer) — do NOT reference or emit \`.mp3\`/\`.wav\`/\`.ogg\` binary files; they cannot be generated and will 404.
    ❌ Binary asset files (.png, .jpg, .svg, .mp3, .wav, etc.) cannot be written from generated code — use the approaches above.

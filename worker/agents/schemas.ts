import z from 'zod';
import { InterviewSpecSchema, type InterviewSpec } from './interview/types';

/**
 * Capability tags for roadmap phases. Drives the canonical partial-order
 * lint in roadmapValidator: shell → core-flow → persistence → auth →
 * uploads → payments → email → realtime → admin → analytics → polish.
 */
export const ROADMAP_CAPABILITIES = [
    'shell', 'core-flow', 'persistence', 'auth', 'uploads',
...
export const ImageAssetSchema = z.object({
    path: z.string().describe('Project-relative path the image will be written to and referenced at, e.g. "public/logo.png" or "src/assets/hero.png"'),
    prompt: z.string().describe('Plain-language description of the desired image, detailed enough for a dedicated image model to render it well'),
    ...
    width: z.number().nullable().describe('Desired width in pixels (divisible by 16), or null for the default. Use null if unspecified.'),
    height: z.number().nullable().describe('Desired height in pixels (divisible by 16), or null for the default. Use null if unspecified.'),
    url: z.string().nullable().describe('Public URL of the generated asset. Populated by the system after the image is generated and uploaded; set this to null when authoring the blueprint. Once present, reference the image at this URL (e.g. <img src="{url}">).'),
});
...
export const BlueprintSchema = z.object({
    ...
    implementationRoadmap: z.array(z.object({
        ...
        id: z.string().nullable().describe('Stable kebab-case slug for this phase, e.g. "payments-checkout"'),
        dependsOn: z.array(z.string()).nullable().describe('ids of roadmap phases that must be completed before this one can start (e.g. payments depends on the auth phase and the phase that creates something to sell)'),
        capability: z.enum(ROADMAP_CAPABILITIES).nullable().describe('The capability this phase primarily delivers'),
        satisfies: z.array(z.string()).nullable().describe('Requirement ids (US-* / AC-*) from the product specification this phase implements, when one exists'),
        demonstrable: z.string().nullable().describe('One behavior a person can verify in the preview after this phase, e.g. "sign up, create a booking, see it persist after refresh"'),
    })).describe('Phases of the implementation roadmap, in dependency order'),
    ...
    imageAssets: z.array(ImageAssetSchema).nullable().describe('Real image assets the application genuinely needs (logos, hero images, illustrations), or null/[] if none. Declared here so they are generated by dedicated image models before the first build and referenced as real files. Omit decorative chrome that CSS/SVG can produce. Keep the list tight (typically 0-5).'),
...
export type RoadmapPhase = z.infer<typeof BlueprintSchema>['implementationRoadmap'][number];

/**
 * Prompt-serialization schema: the blueprint plus the interview spec the
 * system attaches after inference. The spec is never model-authored, so it
 * lives outside the inference schema but inside everything downstream —
 * state, prompts (via schema-driven serialization), and the frontend.
 */
export const BlueprintWithSpecSchemaLite = BlueprintSchemaLite.extend({
    spec: InterviewSpecSchema.nullable().optional(),
});
...
/**
 * The blueprint as it lives in state and prompts: the model-generated
 * document plus the structured interview spec attached after inference
 * (null/absent for builds without an interview).
 */
export type Blueprint = z.infer<typeof BlueprintSchema> & { spec?: InterviewSpec | null };

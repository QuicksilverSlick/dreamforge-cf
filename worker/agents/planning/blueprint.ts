...
<INSTRUCTIONS>
    ...
    **Visual Assets - Two Tracks:**
    1. **Generated image assets (\`imageAssets\` manifest):** For the few *real raster images* the product genuinely needs — brand logo, hero/banner image, key illustrations, avatars — declare each one in the \`imageAssets\` array (\`path\`, \`prompt\`, \`purpose\`, optional \`width\`/\`height\`). These are produced by dedicated image models *before the first build* and made available as real files at the given \`path\`, so the coder can reference them directly (e.g. \`<img src="/logo.png">\`). Keep this list tight (typically 0-5); only include assets that materially improve the product. Write detailed, descriptive prompts.

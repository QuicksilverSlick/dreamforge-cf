...
<INSTRUCTIONS>
    ...
    **Visual Assets - Two Tracks:**
    1. **Generated image assets (\`imageAssets\` manifest):** For the few *real raster images* the product genuinely needs — brand logo, hero/banner image, key illustrations, avatars — declare each one in the \`imageAssets\` array (\`path\` identifying the asset, \`prompt\`, \`purpose\`, optional \`width\`/\`height\`; do NOT set \`url\`). These are produced by dedicated image models *before the first build*; the system then populates each entry's \`url\` with a public URL the coder references directly (e.g. \`<img src="{url}">\`). Keep this list tight (typically 0-5); only include assets that materially improve the product. Write detailed, descriptive prompts.
    2. **Everything else — use code, not files:**
    ...
    ❌ Do NOT list \`.png\`/\`.jpg\`/\`.svg\`/\`.gif\` files in a phase's files list — declared \`imageAssets\` are generated by the image pipeline, not written by the coder; decorative chrome should be CSS/SVG/canvas.
</INSTRUCTIONS>
...
export interface BlueprintGenerationArgs {
    ...
    /** Text extracted from files the user attached to this build (transient). */
...
/**
 * Render the user's attached documents as blueprint context. Each is fenced
 * and explicitly framed as SOURCE MATERIAL, never instructions — the same
 * posture as the reference-site content block, since attachment text is
 * untrusted user input.
 */
const KNOWN_KINDS: ReadonlySet<string> = new Set(['image', 'text', 'document', 'data']);

function formatAttachedDocuments(docs: AttachedDocument[]): string {
...
export async function generateBlueprint({ env, inferenceContext, query, language, frameworks, templateDetails, templateMetaInfo, images, interviewSpec, referenceSite, attachedDocuments, stream }: BlueprintGenerationArgs): Promise<Blueprint> {
    try {
        ...
        const clientRequest = `CLIENT REQUEST: "${query}"${interviewSpec ? formatInterviewSpec(interviewSpec) : ''}${referenceSite ? formatReferenceSite(referenceSite) : ''}${attachedDocuments && attachedDocuments.length > 0 ? formatAttachedDocuments(attachedDocuments) : ''}`;

...
export const PROMPT_UTILS = {
     ...
     * prompt. CommonMark link reference definitions (e.g. `[a]: # "..."`)
     * render as nothing in the chat UI but are passed verbatim to the model,
     * making them a vehicle for hidden prompt-injection instructions such as
     * markdown image exfiltration beacons. Strip any such line so injected
     * directives never reach the prompt.
     */
...
export const STRATEGIES_UTILS = {
    INITIAL_PHASE_GUIDELINES: `**First Phase: Stunning Frontend Foundation & Visual Excellence**
        ...
        * **🚨 MANDATORY — REPLACE THE TEMPLATE PLACEHOLDER HOME PAGE:** The template ships a placeholder entry/home file that renders a "Creating your app…" / Durable-Object-Counter boilerplate (the \`TemplateDemo\` component, gated by \`HAS_TEMPLATE_DEMO\`). Your **FIRST phase MUST completely replace the entire contents** of that entry file — \`src/pages/HomePage.tsx\` for React/Vite templates, or \`public/index.html\` for static templates — with the real application UI. After phase 1, that file MUST NOT import or render \`TemplateDemo\` / reference \`HAS_TEMPLATE_DEMO\`, and the app MUST NOT show the "Creating your app" / "please wait" placeholder. Rewrite the file in place (do not delete it, do not route around it to a different file). **The very first deploy must render the actual application — never the placeholder.** A build whose first deploy still shows the placeholder is considered broken.`,

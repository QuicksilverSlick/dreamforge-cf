...
describe('validateAndCleanBootstrapCommands', () => {
	...
	it('deduplicates equivalent package operations, latest wins', () => {
		const { validCommands, deduplicated } = validateAndCleanBootstrapCommands([
			'bun add react react-dom',
			'bun add react-dom react',
		]);
		expect(validCommands).toEqual(['bun add react-dom react']);
		expect(deduplicated).toBe(1);
	});

	it('caps the history at maxCommands, keeping the most recent', () => {
		const commands = Array.from({ length: 5 }, (_, i) => `bun add pkg-${i}`);
		const { validCommands } = validateAndCleanBootstrapCommands(commands, 3);
		expect(validCommands).toEqual(['bun add pkg-2', 'bun add pkg-3', 'bun add pkg-4']);
	});
});

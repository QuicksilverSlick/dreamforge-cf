...
export default ({ mode }) => {
    ...
    return defineConfig({
        ...
        server: {
            ...
            host: '0.0.0.0',
            allowedHosts: true,
            // HMR configuration for sandbox environment
            hmr: {
                overlay: true,
            },
        },
...
export function normalizeViteConfig(
    ...
    resolve: {
        ...
        if (normalized.includes(configOpenPattern)) {
            ...
            changes.push('Added @ and @shared path aliases');
        }
    }

    // Ensure server config exists with proper port and host
    ...
    server: {
        ...
        if (normalized.includes(configOpenPattern)) {
            ...
            changes.push('Added server config with port, host, and allowedHosts');
        }
    } else {

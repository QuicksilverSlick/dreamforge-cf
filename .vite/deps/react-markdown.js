...
import { asciiAlpha, asciiAlphanumeric, asciiAtext, asciiControl, asciiDigit, asciiHexDigit, asciiPunctuation, blankLine, classifyCharacter, codes, combineExtensions, constants, factorySpace, markdownLineEnding, markdownLineEndingOrSpace, markdownSpace, normalizeIdentifier, ok, push, resolveAll, splice, toString, types, unreachable, values } from "./blank-line-B8UdZhW5.js";
import { esm_default, stringify as stringify$1 } from "./esm-wU2t5WA2.js";
	...
	*/
	function contStart(code$1) {
		if (markdownSpace(code$1)) {
			...
			return factorySpace(effects, contBefore, types.linePrefix, self.parser.constructs.disable.null.includes("codeIndented") ? void 0 : constants.tabSize)(code$1);
		}
	...
	*/
	function infoBefore(code$1) {
		if (code$1 === codes.eof || markdownLineEnding(code$1)) {
			...
			return self.interrupt ? ok$1(code$1) : effects.check(nonLazyContinuation, atNonLazyBreak, after)(code$1);
		}
	...
	function tokenizeCloseStart(effects$1, ok$2, nok$1) {
		...
		*/
		function start$1(code$1) {
			...
			return markdownSpace(code$1) ? factorySpace(effects$1, beforeSequenceClose, types.linePrefix, self.parser.constructs.disable.null.includes("codeIndented") ? void 0 : constants.tabSize)(code$1) : beforeSequenceClose(code$1);
		}
		/**
...
function tokenizeNonLazyContinuation(effects, ok$1, nok) {
	...
	*/
	function lineStart(code$1) {
		return self.parser.lazy[self.now().line] ? nok(code$1) : ok$1(code$1);
	}
}
...
function tokenizeContinuation(effects, ok$1, nok) {
	...
	*/
	function prefixed(code$1) {
		...
		ok(self.parser.constructs.disable.null, "expected `disable.null` to be populated");
		const tail = self.events[self.events.length - 1];
		if (!self.parser.constructs.disable.null.includes("codeIndented") && tail && tail[1].type === types.linePrefix && tail[2].sliceSerialize(tail[1], true).length >= constants.tabSize) return ok$1(code$1);
		return effects.interrupt(self.parser.constructs.flow, nok, ok$1)(code$1);
	}
}
...
function factoryLabel(effects, ok$1, nok, type, markerType, stringType) {
	...
	*/
	function atBreak(code$1) {
		if (size > constants.linkReferenceSizeMax || code$1 === codes.eof || code$1 === codes.leftSquareBracket || code$1 === codes.rightSquareBracket && !seen || code$1 === codes.caret && !size && "_hiddenFootnoteSupport" in self.parser.constructs) return nok(code$1);
...
function tokenizeDefinition(effects, ok$1, nok) {
	...
	*/
	function before(code$1) {
		ok(code$1 === codes.leftSquareBracket, "expected `[`");
		return factoryLabel.call(self, effects, labelAfter, nok, types.definitionLabel, types.definitionLabelMarker, types.definitionLabelString)(code$1);
	}
	/**
	* After label.
	*
	...
	*/
	function cdataOpenInside(code$1) {
		...
		if (code$1 === value.charCodeAt(index$1++)) {
			effects.consume(code$1);
			if (index$1 === value.length) return self.interrupt ? ok$1 : continuation;
			return cdataOpenInside;
		}
		return nok(code$1);
	}
...
function tokenizeHtmlText(effects, ok$1, nok) {
	...
	*/
	function lineEndingAfter(code$1) {
		ok(self.parser.constructs.disable.null, "expected `disable.null` to be populated");
		return markdownSpace(code$1) ? factorySpace(effects, lineEndingAfterPrefix, types.linePrefix, self.parser.constructs.disable.null.includes("codeIndented") ? void 0 : constants.tabSize)(code$1) : lineEndingAfterPrefix(code$1);
	}
	/**
...
function tokenizeLabelEnd(effects, ok$1, nok) {
	...
	/** @type {boolean} */
	let defined;
	while (index$1--) if ((self.events[index$1][1].type === types.labelImage || self.events[index$1][1].type === types.labelLink) && !self.events[index$1][1]._balanced) {
...
function tokenizeReferenceFull(effects, ok$1, nok) {
	...
	*/
	function referenceFull(code$1) {
		...
		return factoryLabel.call(self, effects, referenceFullAfter, referenceFullMissing, types.reference, types.referenceMarker, types.referenceString)(code$1);
	}
	/**
	...
	*/
	function referenceFullAfter(code$1) {
		return self.parser.defined.includes(normalizeIdentifier(self.sliceSerialize(self.events[self.events.length - 1][1]).slice(1, -1))) ? ok$1(code$1) : nok(code$1);
	}
	/**
...
function tokenizeLabelStartImage(effects, ok$1, nok) {
	...
	*/
	function after(code$1) {
		/* c8 ignore next 3 */
		return code$1 === codes.caret && "_hiddenFootnoteSupport" in self.parser.constructs ? nok(code$1) : ok$1(code$1);
	}
}
...
function tokenizeListStart(effects, ok$1, nok) {
	...
	function start(code$1) {
		...
		const kind = self.containerState.type || (code$1 === codes.asterisk || code$1 === codes.plusSign || code$1 === codes.dash ? types.listUnordered : types.listOrdered);
		if (kind === types.listUnordered ? !self.containerState.marker || code$1 === self.containerState.marker : asciiDigit(code$1)) {
			...
			if (!self.interrupt || code$1 === codes.digit1) {
				effects.enter(types.listItemPrefix);
	...
	function inside(code$1) {
		...
		if ((!self.interrupt || size < 2) && (self.containerState.marker ? code$1 === self.containerState.marker : code$1 === codes.rightParenthesis || code$1 === codes.dot)) {
...
function tokenizeListContinuation(effects, ok$1, nok) {
	...
	function onBlank(code$1) {
		...
		return factorySpace(effects, ok$1, types.listItemIndent, self.containerState.size + 1)(code$1);
	}
	/** @type {State} */
	...
	function notInCurrentItem(code$1) {
		...
		self.interrupt = void 0;
		ok(self.parser.constructs.disable.null, "expected `disable.null` to be populated");
		return factorySpace(effects, effects.attempt(list$1, ok$1, nok), types.linePrefix, self.parser.constructs.disable.null.includes("codeIndented") ? void 0 : constants.tabSize)(code$1);
	}
}
/**
* @this {TokenizeContext}
...
function tokenizeIndent(effects, ok$1, nok) {
	...
	function afterPrefix(code$1) {
		ok(self.containerState, "expected state");
		const tail = self.events[self.events.length - 1];
		return tail && tail[1].type === types.listItemIndent && tail[2].sliceSerialize(tail[1], true).length === self.containerState.size ? ok$1(code$1) : nok(code$1);
	}
}
/**
* @this {TokenizeContext}
...
function tokenizeSetextUnderline(effects, ok$1, nok) {
	...
	*/
	function start(code$1) {
		...
		while (index$1--) if (self.events[index$1][1].type !== types.lineEnding && self.events[index$1][1].type !== types.linePrefix && self.events[index$1][1].type !== types.content) {
			paragraph$1 = self.events[index$1][1].type === types.paragraph;
			break;
		}
			...
			*/
			function handleMapOfConstructs(map) {
				...
				function start(code$1) {
					...
					const list$2 = [...Array.isArray(left) ? left : left ? [left] : [], ...Array.isArray(all) ? all : all ? [all] : []];
...
function post(tree, options) {
	...
	function transform(node, index$1, parent) {

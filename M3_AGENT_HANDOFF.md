...
7. 🔲 Other deferred base helpers, IF reached by behavior-internal flows (conversation tools: `regenerateFile`/`generateFiles`/`updateSlideManifest`, `readFiles`/`listFiles`, `updateProjectName`/`updateBlueprint`, `saveExecutedCommands`, `syncPackageJsonFromSandbox`, `generateReadme`, `initializeAsync`, `updateBootstrapScript`, base `getLogs`). These are NOT on the controller/WS surface — port only when wiring the conversation-processor tools reveals a concrete call. Grep the behavior + UserConversationProcessor tool handlers before porting any.

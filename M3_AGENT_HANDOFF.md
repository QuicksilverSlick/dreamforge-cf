...
5. 🔲 **`fetch`/`handleWebhook`** — the new agent needs the runtime-error webhook subsystem. simpleGen overrides `fetch` (2075) to route `/webhook/*` → `handleWebhook` → `processWebhookEvent` → `handleRuntimeErrorWebhook`, plus `generateWebhookUrl` (2091). Upstream `codingAgent.ts` has NO `fetch`/webhook (handles it elsewhere) — so this is a **fork-specific** port from simpleGen, not upstream. Note: the behavior already polls runtime errors via `fetchRuntimeErrors` → `getInstanceErrors`, so confirm whether the webhook push is still required or is supplementary before porting. State-coupled (`this.state.hostname`, broadcast) — adapt to behavior accessors.
6. 🔲 Other deferred base helpers the new live agent reveals it needs: `saveExecutedCommands`, `syncPackageJsonFromSandbox`, `regenerateFile`/`regenerateFileByPath`/`generateFiles`/`updateSlideManifest`, `listFiles`/`readFiles`, `updateProjectName`/`updateBlueprint`, `deployPreview`, `generateReadme`, `initializeAsync`, `updateBootstrapScript`, base `getLogs`. Port as the live `CodeGeneratorAgent` reveals each need (grep the new agent + behavior call sites).
7. 🔲 GitVersionControl + Vault adaptations 2b.13 marked transitional.
8. 🛑 **HELD — flip `worker/index.ts` binding** `SmartCodeGeneratorAgent` → new `CodeGeneratorAgent`. Outward-facing production cutover; **requires explicit user sign-off** before pushing. Rebase onto `main` (`6756f7b`, 1 clean non-conflicting commit — touches `imagesRoutes.ts`/`screenshots/controller.ts`, NOT agent code) before/around this.

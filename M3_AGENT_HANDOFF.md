...
**Status:** commit 2b COMPLETE; **commit 3 binding FLIP landed in code** (`aed32fc`, main merged `56c843d`) — `CodeGenObject` now binds the new behavior-based `CodeGeneratorAgent`. Test gate **191 / 1 / 2 (BYOP)** throughout. **Remaining go-live step: the production `wrangler deploy`, which needs `.prod.vars` (not in the agent env)** — run it where prod credentials exist, or merge PR #49. Then commit 4 = delete simpleGen/smartGen + legacy websocket.ts + dead webhook code. See §10.
...
10. ✅ **DONE — binding FLIPPED (`aed32fc`)** + main merged (`56c843d`). `worker/index.ts` binds `CodeGenObject` → new `CodeGeneratorAgent` (codingAgent.ts), `getAgentStub`/`cloneAgent` retyped to `DurableObjectStub<CodeGeneratorAgent>`. Typecheck confirmed the new agent satisfies every controller + WS call site. **No wrangler migration needed** (CF best-practice, researched: swapping the impl behind the same class name + binding is a code update, not a migration; backwards-compat with stored state handled by `migrateStateIfNeeded` → `StateMigration.migratePhasic`; both old/new SQLite-backed). `origin/main` (#50 `/api/generated/*` hotfix) merged in so a deploy won't regress it. Gates green (191/1/2).

    **⏳ Production deploy is the remaining go-live action — requires `.prod.vars`.** `npm run deploy` (`bun --env-file .prod.vars scripts/deploy.ts`) needs the production secrets/Cloudflare credentials file, which is NOT present in the dev/agent environment. The code cutover is complete and pushed to PR #49; the actual `wrangler deploy` to production must run where `.prod.vars` exists (the project owner's machine / CI), or via merging PR #49 if a deploy pipeline is wired. Per §4.4 the sandbox layer is still broken pre-M4, so validate on `wrangler dev` after deploy, not by assuming prod works.

### Commit 4 (next) — cleanup now that the new agent is live
Delete `simpleGeneratorAgent.ts` + `smartGeneratorAgent.ts` (orphaned — no references outside their own files), the legacy `./websocket.ts` handler (replaced by `codingAgentWebsocket.ts`), the dead webhook methods, and clean up `CodeGenState`/the deprecated-required-field pattern (§4.2). Rename `codingAgentWebsocket.ts` → `websocket.ts` if desired once the legacy one is gone.

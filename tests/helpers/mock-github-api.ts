...
import { MOCK_REPOSITORIES, type MockRepository } from '../fixtures/github-repositories';
import { MOCK_TOKEN_VALIDATION_RESPONSES } from '../fixtures/github-tokens';

interface MockGitHubOptions {
  /**
   * Accepts either the keyed `MOCK_REPOSITORIES` record or a plain array of
   * `MockRepository` objects so callers can pass ad-hoc repo lists.
   */
  repositories?: Record<string, MockRepository> | MockRepository[];
...
function toRepoArray(repos: Record<string, MockRepository> | MockRepository[]): MockRepository[] {
...
export function mockGitHubAPI(options: MockGitHubOptions = {}): void {
  ...
  fetchMock.get('https://api.github.com')
    ...
    .reply(() => {
      ...
      const repoArray = toRepoArray(repositories);
  ...
  fetchMock.get('https://api.github.com')
    ...
    .reply((request) => {
      ...
      const repo = toRepoArray(repositories).find(r => r.name === repoName);
